...
export function compute_scaled_coordinates(atoms, img_size, angstroms_per_pixel) {
    ...
    // Центр моделі: центр bounding-box (краще для фрагментів типу "A.B")
    ...
    // fallback, якщо раптом координати биті
    ...
    const coords = atoms.map(a => ([
        (a.x - centerX) * scale + cx,
        (a.y - centerY) * scale + cy
    ]));
...
export function draw_atoms(img, atoms, coords, scale, background_gray, opts = {}) {
    ...
    for (let i = 0; i < atoms.length; i++) {
        ...
        // Опорна ціла точка для меж обходу (щоб не гасати по всій картинці)
        ...
        for (let y = (y0i - R); y <= (y0i + R); y++) {
            if (y < 0 || y >= img.h) continue;
            const dy = (y - y0);

            for (let x = (x0i - R); x <= (x0i + R); x++) {
                if (x < 0 || x >= img.w) continue;

                const dx = (x - x0);
                const dist2 = dx * dx + dy * dy;

                const v = intensity * Math.exp(-dist2 / sig2);
                const p = (y * img.w + x);
        ...
        const cs2 = 2 * core_sigma * core_sigma;

        for (let y = (y0i - Rc); y <= (y0i + Rc); y++) {
            if (y < 0 || y >= img.h) continue;
            const dy = (y - y0);

            for (let x = (x0i - Rc); x <= (x0i + Rc); x++) {
                if (x < 0 || x >= img.w) continue;

                const dx = (x - x0);
                const dist2 = dx * dx + dy * dy;

                const add = core_intensity * Math.exp(-dist2 / cs2);
                const p = (y * img.w + x);
                img.a[p] += add;
            }
        }
